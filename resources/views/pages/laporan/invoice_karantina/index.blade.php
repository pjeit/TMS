...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.show', function(e){
            ...
            let url = `laporan_invoice_karantina/load_data?tgl_mulai=${tgl_mulai}&tgl_akhir=${tgl_akhir}&customer=${customer}&status=${status}`;
                ...
                if(datas.result == 'success'){
                    ...
                    if(data.length > 0){
                        for (let i = 0; i < data.length; i++) {
                            var row = $("<tr></tr>");
                            row.append(`<td>[${data[i].get_customer.kode}] ${data[i].get_customer.nama}</td>`);
                            row.append(`<td>${data[i].no_invoice_k}</td>`);
                            ...
                            row.append(`<td>${moneyMask(data[i].sisa_tagihan)}</td>`);
                            $("#result").append(row);
                        }
                    }
                }
