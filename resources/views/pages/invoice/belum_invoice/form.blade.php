                    ...
                    @foreach ($data as $key => $item)
                        <tr id="0">
                            ...
                            <td style="text-align:right">
                                ...
                                @endphp
                                @foreach ($item->sewaOperasionalInvoice as $i => $oprs)
                                ...
                                <input type="hidden" class="cek_detail_addcost" id_sewa="{{ $item->id_sewa }}" name="detail[{{ $item->id_sewa }}][addcost_details]" id="detail_addcost_{{ $item->id_sewa }}" value="{{ json_encode($item->sewaOperasionalInvoice) }}" />
                    ...
                    $('#tabel_addcost > tbody:last-child').append(
                        `
                            <tr id="${index}" id_addcost="${item.id}">
                                ...
                                <td>
                                    <input type="text" id="addcost_total_operasional_${item.id}" id_add_cost="${item.id}" value="${moneyMask(item.total_operasional)}" class="form-control numaja uang hitungBiaya hitungAddCost" ${is_readonly} />
                                </td>
