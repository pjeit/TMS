        ...
        <div class="m-3" style="overflow-x:auto; overflow-y:hidden">
            ...
            <table class="table table-hover table-bordered table-striped" width='100%' id="table_invoice">
                ...
                @isset($data)
                        <tr id="0">
                            <td> 
                                {{ $data->getCustomer->nama }} 
                                <input type="hidden" name="detail[{{ $data->id_sewa }}][id_customer]" value="{{ ($data->id_customer) }}" />
                                <input type="hidden" name="detail[{{ $data->id_sewa }}][id_jo_hidden]" value="{{ $data->id_jo }}" />
                                <input type="hidden" name="detail[{{ $data->id_sewa }}][id_jo_detail_hidden]" value="{{ $data->id_jo_detail }}" />
                            </td>
                            <td> {{ $data->nama_tujuan }} <br> ({{ date("d-M-Y", strtotime($data->tanggal_berangkat)) }})</td>
                            <td>
                                @php
                                    $driver = '';
                                    if($data->id_supplier){
                                        $driver = 'DRIVER REKANAN ' . '('.$data->namaSupplier.')';
                                    }else{
                                        $driver = $data->no_polisi . ' ('.$data->getKaryawan->nama_panggilan.')'; 
                                    }
                                @endphp
                                    {{ $driver }}
                            </td>
                            <td> <span id="no_kontainer_text_{{ $data->id_sewa }}">{{ isset($data->id_jo_detail)? $data->getJOD->no_kontainer:$data->no_kontainer }}</span> <br> <span id='no_seal_text_{{ $data->id_sewa }}'>{{ $data->seal_pelayaran }}</span> </td>
                            <td style="text-align:right" id="muatan_satuan_{{ $data->id_sewa }}">
                                {{ (isset($data->jumlah_muatan ))? number_format($data->jumlah_muatan,2)  : "-" }}
                                <input type="hidden" class="muatan_satuan" name='detail[{{ $data->id_sewa }}][muatan_satuan]' id='muatan_satuan_{{ $data->id_sewa }}' value="{{(isset($data->jumlah_muatan ))?$data->jumlah_muatan : 0}}">
                            </td>
                            <td style="text-align:right" id="tarif_{{ $data->id_sewa }}">{{ number_format($data->total_tarif) }}</td>
                            ...
                            <td style="text-align:right">
                                <span id='diskon_text_{{ $data->id_sewa }}'></span>
                            </td>
                            <td style="text-align:right">
                                <span id='subtotal_text_{{ $data->id_sewa }}'>{{ number_format($total_addcost+$data->total_tarif) }}</span>
                                <input type="hidden" class="hitung_subtotal subtotal_hidden_{{ $data->id_sewa }} {{ $data->deskripsi }}" name='detail[{{ $data->id_sewa }}][subtotal]' id='subtotal_hidden_{{ $data->id_sewa }}' value="{{ $total_addcost+$data->total_tarif }}">
                            </td>
                            <td><span id="catatan_text_{{ $data->id_sewa }}">{{ $data->catatan }}</span>
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][nama_tujuan]' id='nama_tujuan_hidden_{{ $data->id_sewa }}' value="{{ $data->nama_tujuan }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][driver]' id='driver_hidden_{{ $data->id_sewa }}' value="{{ $driver }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][tgl_berangkat]' id='tgl_berangkat_hidden_{{ $data->id_sewa }}' value="{{ date("d-M-Y", strtotime($data->tanggal_berangkat)) }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][no_kontainer]' id='no_kontainer_hidden_{{ $data->id_sewa }}' value="{{ $data->no_kontainer }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][no_seal]' id='no_seal_hidden_{{ $data->id_sewa }}' value="{{ $data->seal_pelayaran }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][no_sj]' id='no_sj_hidden_{{ $data->id_sewa }}' value="{{ $data->no_surat_jalan }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][tarif]' id='tarif_hidden_{{ $data->id_sewa }}' value="{{ $data->total_tarif }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][catatan]' id='catatan_hidden_{{ $data->id_sewa }}' value="{{ $data->catatan }}">
                                <input type="hidden" name='detail[{{ $data->id_sewa }}][diskon]' id='diskon_hidden_{{ $data->id_sewa }}' >
                            </td>
                            <td>
